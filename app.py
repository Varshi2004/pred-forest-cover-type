...
numerical_features = [Elevation, Aspect, Slope, Horizontal_Distance_To_Hydrology, Vertical_Distance_To_Hydrology, Horizontal_Distance_To_Roadways,Hillshade_9am, Hillshade_Noon, Hillshade 3pm, Horizontal_Distance_To_Fire_Points]
numerical_features_scaled = scaler.fit_transform((numerical_features])

#Convert soil type to a dataframe with 40 columns
soil_type_list = list(soil_type)
soil_type_df = pd.DataFrame((soil_type_list], columns=[f'Soil_Type{i}' for i in range(1, 41)])

#Concatenate the numerical features and soil type dataframes
